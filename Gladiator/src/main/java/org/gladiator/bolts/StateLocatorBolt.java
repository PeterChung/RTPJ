...
import org.gladiator.utils.BingMapsLookup;
 ...
 *            -- Remove startTick from the Output
 *            -- rearranged outputs
 */
public final class StateLocatorBolt extends BaseRichBolt {
    ...
    public final void declareOutputFields(final OutputFieldsDeclarer outputFieldsDeclarer) {
        //outputFieldsDeclarer.declare(new Fields("startTick", "tweet", "state"));
    ...
    public final void execute(final Tuple input) {
        ...
        if (stateOptional.isPresent()) {
            ...
            //Emit the start time, state and the complete tweet to the next Bolt.
            //this._outputCollector.emit(new Values(startTick, status, state));
            this._outputCollector.emit(new Values(status, state));
        }
    }
